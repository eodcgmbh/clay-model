"""
End-to-end flood detection model that combines:
1. Clay encoder (frozen) - extracts embeddings from raw images
2. Decoder - upsamples embeddings back to image resolution
3. Binary classifier - predicts flood/no-flood

This unified model processes raw images directly without pre-computed embeddings.
"""
...
class EndToEndFloodClassifier(L.LightningModule):
    """
    End-to-end Lightning module for flood detection that:
    1. Encodes raw images using Clay encoder (frozen)
    ...
    def __init__(self,
        ...
        """Initialize the end-to-end classifier."""
        super().__init__()
        ...
        fs.get(ckpt_s3_path, ckpt_local_path)
        ...
        if freeze_encoder:
            ...
            print(f"🔓 Clay encoder trainable")
        ...
        print(f"🏗️  EndToEndFloodClassifier initialized:")
        print(f"   Input: Raw images {target_size}")
        print(f"   Encoder: Clay (frozen={freeze_encoder})")
    ...
    def encode_image(self, image_batch):
        """
        Encode raw images using Clay encoder.
    ...
    def configure_optimizers(self):
        # Only optimize parameters that require gradients (decoder + classifier)
    ...
    def on_train_start(self):
        """Ensure encoder stays frozen during training."""
        if self.freeze_encoder:
            self.clay_encoder.eval()
    
    def on_train_epoch_start(self):
        """Ensure encoder stays frozen at the start of each epoch."""
        if self.freeze_encoder:
            self.clay_encoder.eval()
